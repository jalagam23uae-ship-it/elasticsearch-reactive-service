...
public class QueryTransformService {
    ...
    public ObjectNode buildFinalQuery(String mappingName, JsonNode inputQuery) {
        ...
        // Parse relationships and table_structures (expected JSON strings or already structured)
        List<FinalQueryTransformer.TableRelation> relations = parseRelations(mappingRow.get(key(mappingRow, "relationships")));
        Map<String, List<FinalQueryTransformer.Column>> tableColumns = parseTableStructures(mappingRow.get(key(mappingRow, "table_structures")));
        ...
        // Use fixed pagination and sort order per request
        return FinalQueryTransformer.transform(inputQuery, finalMap, indexName, 0, 10, "desc");
    }

    private String key(Map<String, Object> row, String desired) {
        String d = desired.toLowerCase(Locale.ROOT);
        for (String k : row.keySet()) {
            if (k != null && k.toLowerCase(Locale.ROOT).equals(d)) return k;
        }
        return desired;
    }
    ...
    private List<FinalQueryTransformer.TableRelation> parseRelations(Object src) {
        try {
            ...
            if (src instanceof String s) {
                ...
                if (node.isArray()) {
                    for (JsonNode n : node) {
                        out.add(new FinalQueryTransformer.TableRelation(
                                text(n, "parent_table"),
                                text(n, "parent_field"),
                                text(n, "child_table"),
                                text(n, "child_field"),
            ...
            if (src instanceof List<?> list) {
                ...
                for (Object o : list) {
                    if (o instanceof Map<?,?> m) {
                        out.add(new FinalQueryTransformer.TableRelation(
                                str(m.get("parent_table")),
                                str(m.get("parent_field")),
                                str(m.get("child_table")),
                                str(m.get("child_field")),
                                str(m.get("type")),
                                toLong(m.get("id")),
                                str(m.get("source"))
                        ));
        ...
        try {
            ...
            if (root.isObject()) {
                root.fields().forEachRemaining(e -> {
                    ...
                    if (arr.isArray()) {
                        for (JsonNode c : arr) {
                            cols.add(new FinalQueryTransformer.Column(
                                    text(c, "name"),
                                    text(c, "type"),
    ...
    private static String text(JsonNode n, String field) {
        ...
        if (v == null) {
            ...
            for (var it = n.fieldNames(); it.hasNext();) {
                String k = it.next();
                if (k.equalsIgnoreCase(field)) return n.get(k).asText();
            }
            return null;
        }
        return v.asText();
